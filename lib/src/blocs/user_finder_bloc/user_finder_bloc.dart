...
class UserVehicleBloc extends Bloc<UserVehicleEvent, UserVehicleState> {
  ...
  UserVehicleBloc({required this.userRepository}) : super(UserVehicleInitial()) {
  ...
  void _onErrorWhileTakingPhoto (ErrorWhileTakingPhoto event, Emitter<UserVehicleState> emit) {
    emit(UserVehicleError("Error al tomar la foto. Intente nuevamente"));
  }

  void _onTicketGenerationRequested (TicketGenerationRequested event, Emitter<UserVehicleState> emit) async {
    emit(GeneratingTicket());
    try {
      ticketService.issueTicket(event.plate, event.watchmanSelectedUser);
      emit(UserVehicleResumeCameraPreview());
      emit(SuccessfulProcess());
    } catch (error) {
      emit(UserVehicleResumeCameraPreview());
      emit(UserVehicleError("Failed to load users: $error"));
    }

  }


  void _onCameraImageTaken (ReadVehiclePlateRequested event, Emitter<UserVehicleState> emit) {
    ...
    emit(UserVehicleSearchError('No hemos encontrado ninguna placa. Intenta manualmente'));
  }

  Future<void> _onFetchVehiclePlateUsersRequested(
      FetchVehicleRelatedUsersRequested event, Emitter<UserVehicleState> emit) async {
  ...
  void _onVehicleUsersPopUpClosed(VehicleUsersPopUpClosed event, Emitter<UserVehicleState> emit) {
    emit(UserVehicleResumeCameraPreview());
